'use client';
import { useAuthenticationStatus, useUserData, useSignOut, useSignInEmailPassword } from '@nhost/nextjs';
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-900 text-white">
        <div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-xl shadow-lg border border-gray-700">
          ...
          <form className="space-y-4" onSubmit={async (e) => { e.preventDefault(); await signInEmailPassword(email, password); }}>
            <div>
            ...
            <button type="submit" className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium transition-colors">Sign In</button>
          </form>
          <p className="text-sm text-gray-400 text-center">Use your Nhost credentials to log in.</p>
        </div>
      </div>
    );
  }
